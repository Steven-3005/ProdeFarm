...
SET NOCOUNT ON
	...
	SET
		...
		VENT_IVA = (SELECT SUM(DETV_TOTAL) AS VENT_SUBTOTAL FROM TD_VENTA_DETALLE WHERE VENT_ID=@VENT_ID AND EST=1) * 0.12,
		VENT_TOTAL = (SELECT SUM(DETV_TOTAL) AS VENT_SUBTOTAL FROM TD_VENTA_DETALLE WHERE VENT_ID=@VENT_ID AND EST=1) + ((SELECT SUM(DETV_TOTAL) AS VENT_SUBTOTAL FROM TD_VENTA_DETALLE WHERE VENT_ID=@VENT_ID AND EST=1) * 0.12)
	WHERE
		VENT_ID = @VENT_ID
...
SUM(TD_VENTA_DETALLE.DETV_CANT) 
AS CANT, TM_PRODUCTO.PROD_NOM, TM_PRODUCTO.PROD_PVENTA, TM_CATEGORIA.CAT_NOM, TM_UNIDAD.UND_NOM, 
                  TM_PRODUCTO.PROD_STOCK, TM_PRODUCTO.PROD_IMG, SUM(TM_VENTA.VENT_TOTAL) AS TOTAL
FROM     TD_VENTA_DETALLE INNER JOIN
         TM_PRODUCTO ON TD_VENTA_DETALLE.PROD_ID = TM_PRODUCTO.PROD_ID INNER JOIN
         TM_CATEGORIA ON TM_PRODUCTO.CAT_ID = TM_CATEGORIA.CAT_ID INNER JOIN
         TM_UNIDAD ON TM_PRODUCTO.UND_ID = TM_UNIDAD.UND_ID INNER JOIN
         TM_VENTA ON TD_VENTA_DETALLE.VENT_ID = TM_VENTA.VENT_ID
WHERE
TM_VENTA.EST= 1
AND TM_PRODUCTO.EST=1
AND TM_VENTA.SUC_ID=@SUD_ID
GROUP BY TD_VENTA_DETALLE.PROD_ID, TM_PRODUCTO.PROD_NOM, 
TM_PRODUCTO.PROD_PVENTA, TM_CATEGORIA.CAT_NOM, TM_UNIDAD.UND_NOM, 
TM_PRODUCTO.PROD_STOCK, TM_PRODUCTO.PROD_IMG
ORDER BY CANT DESC
END

CREATE PROCEDURE SP_L_VENTA_05
@SUC_ID INT
AS
BEGIN
SELECT 
